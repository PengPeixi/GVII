...
def basic_visualize(input_1, gradients, save_path=None, weight=None, cmap='viridis', alpha=0.7):
    ...
    input_ = Image.open('Score_CAM/test_0007.jpg').convert('RGB')
    input_ = input_.resize((354, 354),Image.ANTIALIAS)

    #print(gradients.shape)
    #gradients = gradients.resize((354, 354),Image.ANTIALIAS)
    ...
    ax.set_axis_off()
    #for image, cmap, alpha in [(input_, None, None), (gradients, cmap, alpha)]:
    for image, cmap, alpha in [(gradients, cmap, alpha)]:
        if alpha != None:
            toPIL = transforms.ToPILImage()
            image = toPIL(image)
            image = image.resize((354, 354),Image.ANTIALIAS)
        ax.imshow(image, cmap=cmap, alpha=alpha)
...
#        raise Exception("Invalid target layer name.")
    #target_layer = arch._modules[target_layer_name][-1].fn.fn.proj_in
    target_layer = arch._modules[target_layer_name][0].fn.fn.proj_out
    #target_layer = arch._modules[target_layer_name]
    return target_layer
