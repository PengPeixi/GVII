...
from Score_CAM.cam.scorecam import *
...
# 图片的有间隙拼接
#def image_compose(IMAGE_SIZE, IMAGE_ROW, IMAGE_COLUMN, padding, r_max_lst, image_lst, IMAGE_SAVE_PATH):
def image_compose(IMAGE_SIZE, IMAGE_ROW, IMAGE_COLUMN, padding, r_max_lst, image_lst):
    ...
    for row_item,colunm_item in r_max_lst:
        from_image = image_lst[row_item*IMAGE_ROW+colunm_item]
        to_image.paste(from_image, (colunm_item * IMAGE_SIZE + padding * colunm_item, row_item * IMAGE_SIZE + padding * row_item))
...
def get_answer(p, dataloader):
    ...
    #return [dataloader.dataset.label2ans[item] for item in idx]
...
def get_result_pathVQA(model, dataloader, device, args):
    ...
    with torch.no_grad():
        ...
        t = time.time()
        total_time = 0.0
        for v, q, a, ans_type, q_type, ques_id in iter(dataloader):
            ...
            #eps = 1e-12
            #final_preds = torch.log(torch.sigmoid(preds + fact_pred) + eps)
            final_preds = features
