...
class Trainer(object):
    ...
    def __init__(self, args, model, criterion, optimizer=None, ae_criterion = None):
        ...
        self.criterion_q = criterion.to(self.args.device)
        self.criterion_v = criterion.to(self.args.device)
            ...
            try:
                with torch.no_grad() if eval else contextlib.ExitStack():
                    ...
                    if self.args.autoencoder:
                        features, decoder, fact_pred, v_pred = self.model(sample[0], sample[1])
                    else:
                        features = self.model(sample[0], sample[1])
                    #preds = self.model.classifier(features)

                    #eps = 1e-12
                    #pred_co_all = torch.log(torch.sigmoid(preds + fact_pred) + eps)
                    pred_co_all = features

                    loss_q = self.criterion_q(fact_pred.float(), answers)
                    loss_v = self.criterion_v(v_pred.float(), answers)
                    loss_co_all = self.criterion(pred_co_all.float(), answers)

                    loss = loss_co_all + loss_q + loss_v
